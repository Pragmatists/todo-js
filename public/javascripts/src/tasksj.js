var jGetTasks = function() {
  jCallService({method: 'GET', url: ' http://localhost:3000/tasks'}, jUpdateTasks);
};
  ...
  if(status === 200) {
    ...
    var row = function(task) {
      return '<tr><td>' + task.name + '</td>' 
        ...
        + '<td><A onclick="jDeleteTask(\'' + task.id + '\');">delete</A></td>'
        + '</tr>';
    };
...
var jDeleteTask = function(taskId) {
  jCallService({method: 'DELETE', url: 'http://localhost:3000/tasks/' + taskId}, jUpdateMessage);
};
